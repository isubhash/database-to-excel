    ...
    if (is_admin()) {
        ...
        if (isset($_POST["tbl_name"])) {
            ...
            if ($IsValidTableName == 1) {
                $con = mysqli_connect($wpdb->dbhost, $wpdb->dbuser, $wpdb->dbpassword);
                mysqli_select_db($con, $wpdb->dbname) or die("Couldn't select database.");
                $sql = "SELECT * FROM $tablename";
                $result = @mysqli_query($con, $sql) or die("Couldn't execute query:<br>" . mysqli_error($con) . '<br>' . mysqli_errno());
                ob_clean();
            ...
            <div style="overflow-x:scroll;">
                <table class="wp-list-table widefat plugins" id="table">
                    ...
                    if (isset($_POST["table_name"])) {
                        ...
                        $i = 1;
                        echo "<tr>";
                        $t = mysqli_connect($wpdb->dbhost, $wpdb->dbuser, $wpdb->dbpassword);
                        mysqli_select_db($t, $wpdb->dbname) or die("Couldn't select database.");
                        ...
                        <form action="" method="POST">
                            ...
                            <input class="button button-primary exportbtn" name="exportbtn" type="submit"
                                   name="table_display" value="Export"/>
                        </form>
                    <?php } ?>
                </table>
            </div>
